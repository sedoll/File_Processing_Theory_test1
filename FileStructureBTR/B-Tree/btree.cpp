...
struct BTNode* splitNode(int pos, struct BTNode* node, struct BTNode* parent) { // 현재 노드, 부모 노드, 현재노드에서 넣은 값의 위치를 알고있는 pos를 인자로 받기
    int median; // 분리를 위해 중앙값 알아야 함.
    median = node->num_keys / 2; // 중앙값은 짝수이든 홀수이든 '키개수/2' 
    struct BTNode* right_node; // 분리 한 값을 새로 넣어줄 오른쪽 노드 만듬.(추후 자식이 됨)
    ...
    if (!node->leaf) { // 현재 노드가 리프가 아니면, 자식 담기
        ...
        for (int i = median + 1; i < num_iter; i++) { // 오른쪽 노드에 현재 노드 자식 절반 담기
            ...
            right_node->num_children++; // 새로 채워준 노드의 자식 개수는 증가, 현재 노드에서는 빼기
            node->num_children--;
        }
    }

    // 분리할 때 위로 올릴 부모 노드 처리
    if (node == root) { // 루트면 새 부모 노드 만들어야 됨. 이 부분을 어떻게 해야 할 지 고민
        struct BTNode* new_parent_node;
        new_parent_node = BT_create_node(node->key[median]); // 중앙값 가지고 새 부모 노드 만들기
        node->num_keys--;
        new_parent_node->children[0] = node; // 새부모노드의 왼쪽 자식은 현재 노드
        ...
        for (int i = parent->num_keys; i > pos; i--) { // 부모 노드에 넣어야되니까 거기있던 키 배치 다시하기
            parent->keys[i] = parent->keys[i - 1];
            parent->children[i + 1] = parent->children[i];
        }

        parent->keys[pos] = node->keys[median]; // 부모 노드에 넣어야될 자리에 값 넣기
        parent->num_keys++; // 부모노드에 새로 넣었으니까 키개수 추가, 원래 노드는 키 개수 줄이기
        node->num_keys--;
        parent->children[pos + 1] = right_node; // 왼쪽 노드는 원래 연결되어있으니 오른쪽만 부모노드에 연결.
...
struct BTNode* insertNode(int parent_pos, int val, struct BTNode* node, struct BTNode* parent) { // 삽입할 값,  
    int pos; // 현재 노드에서 키의 위치를 갖고 있어야 함. 왜냐면 넣으려고 하는 값의 위치를 찾아야 하기 때문.
    for (pos = 0; pos < node->num_keys; pos++) {// pos 위치는 0부터 해서, 현재 노드의 키 개수만큼 탐색
        if (val == node->keys[pos].key) { // node의 pos번째 키와 val이 같으면
            printf("Duplicates are not permitted!\n");
            return node;
        }
        else if (val < node->keys[pos].key) { // val이 node의 pos번째 키보다 작으면 그 pos에서 멈춘다.
            break;
        }
    }// 만약 val이 그 node에 있는 값보다 크면 당연히 마지막 pos가 나올 것임.
    if (!node->leaf) { // node leaf 여부가 false이면, leaf가 아니면
        node->children[pos] = insertNode(pos, val, node->children[pos], node); // node의 pos번째 자식 노드에 insertNode 값을 담는다. 재귀로 자식을 탐색하기 위해 또 들어감.
        if (node->num_keys == MAX_KEYS + 1) { // 현재 노드 키 개수가 규칙에서 벗어날거같으면
            node = splitNode(parent_pos, node, parent); // 윗 방향으로 분리를 해야 함.
        }
    }
    else { // leaf일 때의 삽입 로직
        for (int i = node->num_keys; i > pos; i--) { // 끝에서부터 val을 삽입해야 하는 위치에 있는 노드까지의 노드들을 뒤로 땡기는 작업을 한다.
            node->keys[i] = node->keys[i - 1]; // 키가 뒤로 한 칸씩 가는 작업.
            node->children[i + 1] = node->children[i]; // 자식도 마찬가지.
        }

        node->keys[pos].key = val; // val을 삽입해야 하는 위치에 val 삽입.
        node->num_keys++; // 하나 십입했으니 키 개수 증가
        if (node->num_keys == MAX_KEYS + 1) { // leaf 노드가 꽉 찼으면 분리를 해준다.
...
void BT_insert_key(struct BTNode** root, struct KeyPair k)
{
    if (!*root) { // root가 없으면
        ...
        *root = insertNode(0, k.key, *root, *root); // 처음에는 root가 부모이자 리프노드.
    }
}
...
//    int merge_idx = par_node->children[mer_node_pos]->num_children; // merge의 주체 노드쪽에 merge될 키 위치를 지정한다.
...
//
//// 왼쪽에서 빌리는 함수
//void borrowFromLeft(struct BTNode* par_node, int cur_node_pos) { // 부모 노드와 현재 노드 pos위치를 인자로 받음. 이미 현재 노드의 키는 지워졌음. cnt_key는 최소상태 혹은 미만일거임.
...
//
//// 오른쪽에서 빌리는 함수
//void borrowFromRight(struct BTNode* par_node, int cur_node_pos) { // 부모 노드와 현재 노드 pos위치를 인자로 받음. 이미 현재 노드의 키는 지워졌음. cnt_key는 최소상태 혹은 미만일거임.
...
//}
//
//void balanceNode(struct BTNode* node, int child_pos) { // 현재 노드와 자식노드에서의 위치를 인자로 받는 함수(빌리기,병합을 진행)
...
//    for (pos = 0; pos < root->cnt_key; pos++) { // val이 지워져야하니 그 위치를 찾아야 함.현재 노드의 키 개수만큼 탐색
//        if (val == root->key[pos]) { // 현재 노드의 키 배열에서 pos와 val이 같으면
//            flag = true; // 찾았다는 표시
//            break;
//        }
//        else if (val < root->key[pos]) { // 키 배열의 pos 위치 값이 val보다 크면 그 위치에서 멈춰라. 거기에서 아래로 더 들어가야 한다.
//            break;
//        }
//    } // 이게 끝났다는건 그 노드에서 (추가 탐색할) pos위치가 정해졌다는 것 
//    if (flag) { // flag가 true이면 실제로 삭제하는 작업 실시
//        if (root->leaf) {  // 리프에서 삭제해야 하면
//            for (int i = pos; i < root->cnt_key; i++) { // 저장된 pos위치부터 키 개수만큼 탐색
//                node->key[i] = node->key[i + 1]; // 지우려는 키 위치에 그 다음 키로 덮어쓰기
//            }
//            root->cnt_key--;
//        }
//        else { // 내부에서 삭제해야 하면
//            deleteInnerNode(root, pos); //내부 노드의 값을 삭제하는 함수 제작. 현재 노드와 현재노드에서의 값 위치를 인자로 넘김.
//        }
//        return flag;
//    }
//    else { // flag가 false이면(지우려는 값을 못찾은 것)
//        if (root->leaf) { //leaf 노드이면
//            return flag;
//        }
//        else { // 지우려는 값을 못 찾았는데 내부 노드이면 더 내려감.
//            flag = deleteValFromNode(val, root->child[pos]); //val이랑 현재노드의 pos번째 자식 넘겨서 flag 받기
//        }
//    }
//    if (root->child[pos]->cnt_key < min_keys) { // (재귀가 끝나서 다시 올라온뒤)삭제처리했던 자식 노드의 키 개수가 최소숫자 범위 부셔졌을 때
